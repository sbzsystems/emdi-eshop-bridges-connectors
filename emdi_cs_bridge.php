<?php
/*------------------------------------------------------------------------
# EMDI - CSCART BRIDGE by SBZ systems - Solon Zenetzis - version 1.3
# ------------------------------------------------------------------------
...
on  ".$dbprefix."category_descriptions.category_id=".$dbprefix."products_categories.category_id
...
while($alldata = mysql_fetch_array( $data ))
{
		...
		$price=number_format($price, 2, ',', '');
  	 	...
  	 	//$discrate=$alldata['supl_buy_perc'];
	    //$discrate=number_format($discrate, 2, ',', '');	
				
		//$price2=$alldata['supl_buy_price'];
	    //$price2=number_format($price2, 2, ',', '');
		//$price2=$price2 -(($price2*$discrate)/100);                     
		//$price2=$price2 +(($price2*$taxrate)/100);
		//$price2=number_format($price2, 2, ',', '');
		
		
		$price2=$alldata['supl_buy_price'];
	    $supl_buy_perc=$alldata['supl_buy_perc'];
		
		$price2=($price2 - (($price2*$supl_buy_perc)/100));		
		$price2=($price2 + (($price2*$taxrate)/100));
		$price2=number_format($price2, 2, ',', '');
		...
		*/
		...
		echo $product_code_prefix.$id.';'.$name1.';;'.$taxrate.';'.$price.";".$price2.";;".$monada.";".$category.";<br>\n";			 
...
while($alldata = mysql_fetch_array( $data ))
{
		...
		if ($userid==0) {
			echo $id.';'.$onetime_customer_code_prefix.$id.";".$shipping.";". $surcharge.";0;".$hmera.";<br>\n";
		} else {					
			echo $id.';'.$customer_code_prefix.$userid.";".$shipping.";". $surcharge.";0;".$hmera.";<br>\n";
		}
...
//$data = mysql_query("UPDATE ".$dbprefix."orders SET status = 'O' WHERE order_id in (".$orderid.")") or die(mysql_error());
