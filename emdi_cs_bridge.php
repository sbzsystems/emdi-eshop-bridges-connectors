...
$user = 'epigasos_uu';//$config['db_user'];
$password = 'GVaZjV8q';//$config['db_password'];
$db = 'epigasos_ud';//$config['db_name'];
$dbprefix = 'cscart_';
$product_code_prefix='';
$customer_code_prefix='IC';
$onetime_customer_code_prefix='AC';
$lang_code='el';
$tmp_path = $_SERVER['DOCUMENT_ROOT'].'/tmp2';
$timezone=$config->offset; 
$lang_code='el';
//$coid=" and  ".$dbprefix."products.company_id=2";

//////////////
$feature_field1='Barcode';
$feature_field2='Κατασκευαστής';
$vat_field='ΑΦΜ';
...
") or die(mysqli_error($link)); 
	...
	while($alldata = mysqli_fetch_array( $data ))
	{
		...
		if ($feature_field1) {
			...
			pfd.description='".$feature_field1."'
			...
			while($alldata2 = mysqli_fetch_array( $data2 ))
			{ 
				$barcode=$barcode."Barcode$count:".$alldata2['variant'].'\n'; 
				$count++;
			}
		...
		if ($feature_field2) {
			...
			pfd.feature_id=pfv.feature_id and 
			pfvd.variant_id=pfv.variant_id and
			pfd.description='".$feature_field2."'
			and pfv.product_id=".$alldata['product_id'];
			...
			while($alldata2 = mysqli_fetch_array( $data2 ))
			{ 
				$manufacturer="Κατασκευαστής:".$alldata2['variant'].'\n'; 
		...
		$row=$product_code_prefix.$id.';'.$name1.';'.$barcode.$manufacturer.';'.$taxrate.';'.$price.";;;".$monada.";".$category.";;;;".$weight.";1;<br>\n";			 
...
if ($action == 'orders') {
	$data = mysqli_query($link,"SELECT * FROM ".$dbprefix."orders where (status='P' or status='O') and tax_exempt='N' order by order_id,user_id desc"
	
	
	) or die(mysqli_error($link)); //
	//C D I F status='O'
//or status='E' or status='Y'
	...
	while($alldata = mysqli_fetch_array($data ))
	{
		...
		if ($userid==0) {
			echo $id.';'.$onetime_customer_code_prefix.$id.";".$shipping.";".$handling.";".$discount.";".$hmera.";".$notes.";<br>\n";
		} else {					
			echo $id.';'.$customer_code_prefix.$userid.";".$shipping.";".$handling.";".$discount.";".$hmera.";".$notes.";<br>\n";
		}
...
if ($action == 'confirmorder') {
	...
	if ($docid<>124) {
		...
		$data = mysqli_query($link,"UPDATE ".$dbprefix."orders SET status = 'G' WHERE order_id in (".$orderid.")") or die(mysqli_error($link));
		echo $hmera;
