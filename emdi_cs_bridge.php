...
left join ".$dbprefix."tax_rates
on  ".$dbprefix."tax_rates.tax_id=".$dbprefix."products.tax_ids and ".$dbprefix."tax_rates.rate_type in ('P','F')
...
";
	...
	while($alldata = mysqli_fetch_array( $data ))
	{
		...
		//$price='|9:'.$price;
		//$taxrate='';
		echo $product_code_prefix.$id.';'.$name1.' '.$variant_name.';'.$category.';'.$taxrate.';'.$price.";;;".$monada.";;".$imageurl.";".$url.";;".$weight.";1;<br>\n";			 
...
$query="SELECT *,
		...
		(case when user_id=0 or email like '%demo%' then 1 else NULL end) onetime 
...
order by order_id,user_id desc";
	...
	/* status<>'D' and status<>'C' and status<>'N' and status<>'I' and status<>'E' and status<>'F' and status<>'J'  */
	/* and order_id > 294623 */

//echo $query;

	/*
$data = mysqli_query($link,"SELECT * FROM ".$dbprefix."orders where status<>'D' and status<>'C' and status<>'N' and status<>'I' and status<>'E' and status<>'F' and status<>'J' and order_id > 294623 and tax_exempt='N' order by order_id,user_id desc") or die(mysqli_error($link)); //
//C D I F status='O'
*/
	...
	while($alldata = mysqli_fetch_array($data ))
	{
		...
		if (!$onetime) { 
			$custid=$customer_code_prefix.$userid; 
		} else { 
			$custid=$onetime_customer_code_prefix.$id; 
		}
 	//$custid=$onetime.'#'.$userid; 
		
	 		
	if ($address1_s.$postcode_s.$country_s.$state_s.$city_s<>$address1.$postcode.$country.$state.$city)  {
					echo $id.";".$custid.";".$shipping.";".$handling.";".$discount.";".$hmera.";".$notes.";;;;".$custid.'.S'.";".$paymentr.";;;<br>\n";				
	} else {		
					echo $id.";".$custid.";".$shipping.";".$handling.";".$discount.";".$hmera.";".$notes.";;;;;".$paymentr.";;;<br>\n";		
	}
...
// ΤΟ ΕΧΩ ΚΛΕΙΣΕΙ ΔΕΝ ΘΕΛΕΙ ΝΑ ΑΛΛΑΖΕΙ ΤΟ STATUS
