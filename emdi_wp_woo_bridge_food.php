...
/*------------------------------------------------------------------------
		...
		WooCommerce Food
		https://codecanyon.net/item/woocommerce-food-restaurant-menu-food-ordering/25457330?_ga=2.17627129.1912136854.1642143954-675036009.1642143954
...
if ($action == 'order') {
	...
	$query="
		...
		(select woim.meta_value from ".$dbprefix."woocommerce_order_itemmeta woim where woim.order_item_id=ordi.order_item_id and woim.meta_key='_exoptions') as variation_id,
	...
	while($alldata = mysqli_fetch_array( $data ))
	{
		...
		$variation_id = $alldata['variation_id']; 
...
if ($defprodid) {
	$product_id=$defprodid;
}
		
	//	echo $cfld['0'].'#'.  	 	$cfld['1'].'#'.  	 	$cfld['2'].'#'.  	 	$cfld['2']  	 	."\n";
		
		
		echo $product_code_prefix.$product_id.';'.$description.';;;'.$product_quantity.';'.$monada.';'.$amount.';'.$taxrate.';'.$discount.";<br>\n";
		
			 
		//$cfld=json_decode($variation_id,true);
		//var_dump(json_decode($variation_id, true));
		
		
		$cfld=unserialize($variation_id);
	 
		//print_r($cfld);
		foreach ($cfld as $value) {
			//$addon='';
			//print_r($value);
			...
			$proxprice=' + €'.$value['price'];
			$addon=$value['name'].': '.$value['value'].$proxprice;
			
			echo $product_code_prefix.$addonid.';'.$addon.';;;1;;0;'.$taxrate.';'.$discount.";<br>\n";
