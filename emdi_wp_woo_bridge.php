<?php
	...
	if ($action == 'order') {
		...
		,
		(select woim.meta_value from ".$dbprefix."woocommerce_order_itemmeta woim where woim.order_item_id=ordi.order_item_id and woim.meta_key='_tmcartfee_data') as analysis2
		...
		while($alldata = mysqli_fetch_array( $data ))
		{
			...
			$analysis2= unserialize($alldata['analysis2']); 
			//print_r($analysis2);
			//echo 'EID;'.$category.' '.$description.';;;'.$product_quantity.';'.$monada.';'.$amount.';'.$taxrate.';'.$discount.";<br>\n";
			
			foreach ($analysis2 as $type) {
				
				$proprice=number_format($type[0][price], 2, ',', '');
				//$proprice=0;
				//echo $product_code_prefix.$product_id.';'.$type[value].';;;'.$type[quantity].";ΠΡΟΣΘΕΤΟ;0;0;0;<br>\n";
				echo 'PRO;'.$type[0][name].' '.$type[0][value].';;;1;'.$measurementaddon.";".$proprice.";".$taxrate.";0;<br>\n";
		...
		(select itm.meta_value from ".$dbprefix."woocommerce_order_itemmeta itm where itm.order_item_id=ordi.order_item_id and itm.meta_key='_fee_amount') fee

		from ".$dbprefix."woocommerce_order_items ordi
		
		where  ordi.order_id=".$orderid." and ordi.order_item_type='fee'
		...
		while($alldata = mysqli_fetch_array( $data ))
		{
			$order_item_name = $alldata['order_item_name']; 
			$fee = $alldata['fee']; 
			echo 'EID;'.$order_item_name.';;;1;'.$monada.";".$fee.";".$taxrate.";0;<br>\n";
		}
