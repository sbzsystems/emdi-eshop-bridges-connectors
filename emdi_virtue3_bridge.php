<?php
	...
	/*------------------------------------------------------------------------
		# EMDI - VIRTUEMART 3 BRIDGE by SBZ systems - Solon Zenetzis - version 2.0
		# ------------------------------------------------------------------------
	...
	if ($action == 'customers') {
		...
		/*
			...
			."where ".$dbprefix."virtuemart_userinfos.modified_on>'".date('Y-m-d H:i:s', $lastdate)."'
		...
		*/
		...
		."where ".$dbprefix."virtuemart_order_userinfos.modified_on>'".date('Y-m-d H:i:s', $lastdate)."' "
		//."group by phone_2 "
	...
	if ($action == 'orders') {
		$data = mysqli_query($link,"SELECT * FROM ".$dbprefix."virtuemart_orders ord,".$dbprefix."virtuemart_shipmentmethods_el_gr ship,".$dbprefix."virtuemart_order_userinfos ordusrs  where ordusrs.virtuemart_order_id=ord.virtuemart_order_id and  ord.virtuemart_shipmentmethod_id=ship.virtuemart_shipmentmethod_id and ord.order_status in ('P','C') and ord.order_tax<>0 ") or die(mysqli_error()); //
		...
		while($alldata = mysqli_fetch_array( $data ))
		{
			...
			$orofos=$alldata['orofos'];
			$ship=$alldata['shipment_name'];

			
			//	if ($userid) { 
			if(!$userid>0){
				$userid=$id;
				echo $id.';'.$once_customer_code_prefix.$userid.";".$alldata['order_shipment'].";".$alldata['order_payment'].";0;".$hmera.";".$comment." ".$ship." ".$orofos.";<br>\n";
				...
				echo $id.';'.$customer_code_prefix.$userid.";".$alldata['order_shipment'].";".$alldata['order_payment'].";0;".$hmera.";".$comment." ".$ship." ".$orofos.";<br>\n";
			}
